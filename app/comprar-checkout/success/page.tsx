        ...
        >
          ...
          >
            <svg width="40" height="40" viewBox="0 0 40 40" fill="none">
              <path d="M10 20L17 27L30 13" stroke="#2D7D46" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </motion.div>

          <h1 className="text-2xl font-800 text-[#18191F] mb-2">
            ¡Compra confirmada, {firstName}!
          </h1>
          <p className="text-[#666666] mb-8">
            Recibimos tu pago. Te contactaremos pronto para coordinar la entrega.
          </p>

          {/* Order card */}
          {model && (
            <div className="bg-[#F7F7F7] rounded-2xl p-5 text-left mb-6">
              <p className="text-xs font-700 text-[#999999] uppercase tracking-wider mb-3">Tu compra</p>
              <div className="flex items-center gap-3 mb-4 pb-4 border-b border-[#E5E5E5]">
                ...
                <div>
                  <p className="font-700 text-[#18191F]">{model}</p>
                  <p className="text-xs text-[#999999]">Equipo usado certificado</p>
                </div>
              </div>
              <div className="space-y-2 text-sm">
                {orderId && (
                  <div className="flex justify-between">
                    <span className="text-[#666666]">N° de orden</span>
                    <span className="font-600 text-[#18191F] font-mono text-xs">{orderId.slice(0, 8).toUpperCase()}</span>
                  </div>
                )}
                {price && (
                  <div className="flex justify-between pt-2 border-t border-[#E5E5E5]">
                    <span className="font-700 text-[#333333]">Total pagado</span>
                    <span className="font-800 text-[#1B4FFF]">${parseFloat(price).toLocaleString("en-US")} USD</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="text-left mb-8">
            ...
            <div className="space-y-3">
              {[
                ...
                { icon: "📞", text: "Te llamaremos en las próximas 24 horas para coordinar la entrega" },
                { icon: "🚚", text: "Tu Mac llega en 24-48 horas hábiles en Lima" },
                { icon: "🧹", text: "El equipo llega formateado y listo para usar" },
              ].map((item) => (
