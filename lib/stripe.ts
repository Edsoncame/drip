...
export async function createCheckoutSession(
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    metadata: {
      ...
      billing_name: params.customer.name.slice(0, 250),
      billing_phone: params.customer.phone.slice(0, 40),
      billing_company: params.customer.company.slice(0, 250),
      billing_ruc: params.customer.ruc.slice(0, 30),
